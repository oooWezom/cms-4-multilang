<?php if (count($images)): ?>
    <?php foreach($images as $im): ?>
        <?php if (is_file(HOST.Core\HTML::media('images/gallery_images/small/'.$im->image, false))): ?>
            <div class="loadedBlock <?= $im->status == 1 ? 'chk' : ''; ?>" data-image="<?=$im->id; ?>">
                ...
                <div class="loadedControl">
                    <?php if(\Core\User::god() || \Core\User::get_access_for_controller('gallery') == 'edit'): ?>
                        <div class="loadedCtrl loadedCover">
                            <label>
                                ...
                                <ins></ins>
                                <span class="btn btn-success" alt="<?php echo __('На главной'); ?>"><i class="fa fa-bookmark-o"></i></span>
                    ...
                    <div class="loadedCtrl loadedView">
                        <button class="btn btn-primary btnImage" alt="<?php echo __('Просмотр'); ?>" href="<?php echo Core\HTML::media('images/gallery_images/big/'.$im->image); ?>"><i class="fa fa-search-plus"></i></button>
                    </div>
                    <?php if(\Core\User::god() || \Core\User::get_access_for_controller('gallery') == 'edit'): ?>
