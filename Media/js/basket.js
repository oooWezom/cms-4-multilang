$(document).ready(function() {
    ...
    var wb_i, //инпут спиннера
        ...
        wb_x = 0, // копейки - нулей после запятой / если целое число = 0 ****
    ...
    function wb_calcOne(reObj) {
        ...
        $('.wBasket').each(function(index, el) {
            ...
            // wb_t = $(el).find('[data-id="' + reObj.attr('data-id') + '"]');
            wb_t = reObj;
            ...
            if (wb_t.find('.wb_select')) {
                var temp_sel = wb_t.find('.wb_select');
                temp_sel.find('option:selected').prop('selected', false);
                temp_sel.find('[value="' + wb_t.attr(temp_sel.attr('data-equal')) + '"]').prop('selected', true);
            }
            // переназначение радио - если есть
            if (wb_t.find('.wb_check')) {
                var temp_sel = wb_t.find('.wb_check');
                temp_sel.find('input:checked').prop('checked', false);
                temp_sel.find('[value="' + wb_t.attr(temp_sel.attr('data-equal')) + '"]').prop('checked', true);
            }*/
            wb_t = '';
        });
        wb_p = '';
    ...
    $('.' + wb_m).magnificPopup({
        ...
        midClick: true,
        removalDelay: 300,
        mainClass: 'wb_zoomIn' // wb_slideBottom, wb_slideRight, wb_zoomIn
    });
