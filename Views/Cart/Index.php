<?php
use Core\HTML;
use Core\Arr;

?>
<?php if (count($cart)): ?>
    <div class="basket_page_main clearFix">
        <div class="fll basket_form">
            <div form="true" class="wForm regBlock" data-ajax="checkout">
                <div class="wBasketHead">
                    <div class="wBasketTTL"><?php echo __('Оформление заказа')?></div>
                </div>
                <div class="wFormRow">
                    <select data-name="payment" name="payment" id="select10" data-rule-required="true">
                        <option value=""><?php echo __('Выберите способ оплаты')?></option>
                        <?php foreach ($payment as $id => $name): ?>
                            <option value="<?php echo $id; ?>"><?php echo $name; ?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                <div class="wFormRow">
                    <select data-name="delivery" name="delivery" id="select11" data-rule-required="true">
                        <option value=""><?php echo __('Выберите способ доставки')?></option>
                        <?php foreach ($delivery as $id => $name): ?>
                            <option value="<?php echo $id; ?>"><?php echo $name; ?></option>
                        <?php endforeach ?>
                    </select>
                    <input data-name="number" type="text" class="dostavka" name="number"
                           placeholder="<?php echo __('Введите номер отделения Новой Почты')?>" data-rule-required="true">
                </div>
                <div class="wFormRow">
                    <input data-name="name" type="text" name="name" data-rule-bykvu="true" placeholder="<?php echo __('Имя получателя')?>"
                           data-rule-minlength="2" data-rule-required="true">
                    <label><?php echo __('Фамилия и Имя получателя')?></label>
                </div>
                <div class="wFormRow">
                    <input data-name="email" type="email" name="em" data-rule-email="true" placeholder="<?php echo __('E-mail')?>"
                           required="">
                    <label>E-mail</label>
                </div>
                <div class="wFormRow">
                    <input type="tel" data-name="phone" class="tel" name="phone" data-rule-phoneUA="true" maxlength="19"
                           minlength="19" placeholder="<?php echo __('Телефон')?>" data-rule-data-rule-required="truetrue">
                    <label><?php echo __('Телефон')?></label>
                </div>
                <input type="hidden" data-name="lang" value="<?php echo \I18n::$lang; ?>">
                <?php if (array_key_exists('token', $_SESSION)): ?>
                    <input type="hidden" data-name="token" value="<?php echo $_SESSION['token']; ?>"/>
                <?php endif; ?>
                <div class="butt">
                    <button class="wSubmit enterReg_btn"><?php echo __('подтвердить заказ')?></button>
                </div>
            </div>
        </div>
        <div class="wBasket wBasketModule main_b">
            <div class="wBasketWrapp">
                <div class="wBasketHead">
                    <div class="wBasketTTL"><?php echo __('Корзина')?></div>
                </div>
                <div class="wBasketBody">
                    <ul class="wBasketList">
                        <?php $amount = 0; ?>
                        <?php foreach ($cart as $key => $item): ?>
                            <?php $obj = Arr::get($item, 'obj'); ?>
                            <?php if ($obj): ?>
                                <li class="wb_item" data-id="<?php echo $obj->id; ?>"
                                    data-count="<?php echo Arr::get($item, 'count', 1) ?>"
                                    data-price="<?php echo $obj->cost; ?>">
                                    <div class="wb_li">
                                        <?php if (is_file(HOST . HTML::media('images/catalog/medium/' . $obj->image, false))): ?>
                                            <div class="wb_side">
                                                <div class="wb_img">
                                                    <a href="<?php echo HTML::link($obj->alias . '/p' . $obj->id); ?>"
                                                       class="wbLeave">
                                                        <img src="<?php echo HTML::media('images/catalog/medium/' . $obj->image); ?>"/>
                                                    </a>
                                                </div>
                                            </div>
                                        <?php endif; ?>
                                        <div class="wb_content">
                                            <div class="wb_row">
                                                <div class="wb_del"><span title="<?php echo __('Удалить товар')?>"><?php echo __('Удалить товар')?></span>
                                                </div>
                                                <div class="wb_ttl">
                                                    <a href="<?php echo HTML::link($obj->alias . '/p' . $obj->id); ?>"
                                                       class="wbLeave">
                                                        <?php echo $obj->name; ?>
                                                    </a>
                                                </div>
                                            </div>
                                            <div class="wb_cntrl">
                                                <div class="wb_price_one"><p><span><?php echo $obj->cost; ?></span> грн.
                                                    </p></div>
                                                <div class="wb_amount_wrapp">
                                                    <div class="wb_amount">
                                                        <input type="text" class="editCountItem"
                                                               value="<?php echo Arr::get($item, 'count', 1); ?>">
                                                        <span data-spin="plus" class="editCountItem"></span>
                                                        <span data-spin="minus" class="editCountItem"></span>
                                                    </div>
                                                </div>
                                                <div class="wb_price_totl"><p>
                                                        <span><?php echo $obj->cost * Arr::get($item, 'count', 1); ?></span>
                                                        грн.</p></div>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                                <?php $amount += $obj->cost * Arr::get($item, 'count', 1); ?>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </ul>
                </div>
                <div class="wBasketFooter">
                    <div class="wb_footer">
                        <div class="tar wb_footer_tot">
                            <div class="wb_total"><?php echo __('Итого:')?> <span id="topCartAmount"><?php echo $amount; ?></span> грн.
                            </div>
                        </div>
                        <div class="flr wb_footer_go">
                            <div class="wb_gobasket">
                                <a href="<?php echo HTML::link('products'); ?>"
                                   class="wb_butt"><span><?php echo __('Продолжить покупки')?></span></a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
<?php else: ?>
    <p class="emptyCartBlock"><?php echo __('Ваша корзина пуста.')?> <a href="<?php echo HTML::link('products'); ?>"><?php echo __('Начните делать
            покупки прямо сейчас!')?></a></p>
<?php endif ?>