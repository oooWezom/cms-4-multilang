...
$(document).ready(function(){
	...
	$(document).on('click','.dropdownToggle',function(){
		...
		ddMenu.css({top:ddToggle.position().top + ddToggle.outerHeight(), left:ddToggle.position().left});
		if(ddMenu.is('.pull-right')){
			ddMenu.css({left:'auto', right:(ddParent.width() - ddToggle.position().left) - ddToggle.outerWidth()})
		}
		...
		if(ddToggle.is('.dropdownSelect')){
			$('a',ddMenu).on('click',function(){
				...
				if(ddToggle.is('.choiceColorToggle')){
					$('.current-item',ddToggle).css({backgroundColor:subActive.css('background-color')})
				}	
			})	
		}
